...
def normalize_intensity_contrast(image):
    ...
    # Define a target mean intensity and standard deviation (you can adjust these)
    target_mean = 10
    target_std = 2
   
    # Adjust the intensity and contrast
    normalized_image = ((image - mean_intensity) / std_intensity) * target_std + target_mean
...
def load_spn (path, in_shape, verbose):
    ...
    for l,imagePath_ct in enumerate(imagePaths_ct): #load, resize, normalize, etc
        ...
        try:
            ...
            #image_ct = cv2.cvtColor(image_ct, cv2.COLOR_BGR2GRAY)
            #image_ct = cv2.equalizeHist(image_ct)
            #image_ct = cv2.cvtColor(image_ct,cv2.COLOR_GRAY2BGR)
            
            #image_ct = normalize_intensity_contrast(image_ct)
            ...
            #image_pet = normalize_intensity_contrast(image_pet)
...
def load_lidc(path, in_shape, verbose):
    ...
    for l,imagePath_early in enumerate(imagePaths_early): #load, resize, normalize, etc
        ...
        try:
            ...
            #image = normalize_intensity_contrast(image)
    ...
    data_lidc = np.array(data_early, dtype="float")
    labeltemp=labels
    labels_lidc = np.array(labels)
    lb = LabelBinarizer()
    labels = lb.fit_transform(labels) 
    labels = tf.keras.utils.to_categorical(labels, num_classes=2)   
    print("Data and labels loaded and returned")
    return data_lidc, labels

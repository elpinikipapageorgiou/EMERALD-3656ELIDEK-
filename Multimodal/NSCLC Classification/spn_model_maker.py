...
def selector (model_name,in_shape,tune,classes):
    ...
    if model_name == 'xception':
        model = make_xception(in_shape,tune,classes)
    if model_name == 'simple_cnn':
        model = simple_cnn(in_shape,tune,classes)   
...
def simple_cnn(in_shape, tune='auto', classes=2):
    ...
    print("[INFO] Simple CNN Model Compiled!")
    return model
...
def furnish_ffvgg19 (in_shape, tune, classes):
    ...
    early3 = tf.keras.layers.Dropout(0.7)(early3)
    ...
    early4 = tf.keras.layers.Dropout(0.7)(early4)

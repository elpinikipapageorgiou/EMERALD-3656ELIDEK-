...
###Import functions
from particle_functions import DeepFCMx, sig
...
def read_and_process_image(list_of_images):
    ...
    y = [0 if 'class0' in addr else 1 for addr in list_of_images]
...
dataset = create_feature_maps(addrs, num_clusters,pixel_size,feature_layer_model)
...
for train_index, test_index in kf.split(dataset):
    ...
    best_position, concept_evolution  = DeepFCMx(training_dataset,  num_dimensions=num_dimensions, num_particles=num_particles, 
                                                 num_clusters=num_clusters,maxiter=epoch)
    end = time.time()
        ...
        for i in range(num_dimensions):
            for j in range(num_dimensions):
                if i == j:
                    ...
                    sum_temp += best_position[j][i] * testing_row[j]
    ...
    # Check if the array is one-dimensional and fix accordingly
    if testing_last_element_fcm_output.ndim == 1:
        testing_last_element_fcm_output = testing_last_element_fcm_output.reshape(-1, 1)

    # Ensure the correct dimension is selected
    testing_last_element_fcm_output = testing_last_element_fcm_output[:, 0]
    ...
    for i in limits:
        # Ensure temporary_value_results holds valid values
        temporary_value_results = (steady_value_predicted_results > i).astype(float)

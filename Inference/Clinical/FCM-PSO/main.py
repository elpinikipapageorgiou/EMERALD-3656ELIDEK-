...
def remove_indices(user_inputs_dict, best_position, user_inputs):
    ...
    #     user_inputs_dict["previous Stroke"] = user_inputs_dict["previous STROKE"]
    #     del user_inputs_dict["previous STROKE"]   
    ...
    # if "Hypertension" in user_inputs_dict:
    #     user_inputs_dict["Arterial Hypertension"] = user_inputs_dict["Hypertension"]
    #     del user_inputs_dict["Hypertension"]    
    ...
    # st.write(best_position, random_number, hypertension_index)
    
    # if "Dyslipidemia" in user_inputs_dict:
    #     user_inputs_dict["Dislipidemia"] = user_inputs_dict.pop("Dyslipidemia")
        
    # if "Dyslipidemia" in user_inputs_dict:
    #     user_inputs_dict["Dislipidemia"] = user_inputs_dict["Dyslipidemia"]
    #     del user_inputs_dict["Dyslipidemia"]
...
# Counterfactual explanation generation function
def generate_counterfactual_explanation(input_data, interconnection_strength_text, classification):
    counterfactual = []

    if classification == "pathological":
        # Suggest changes to classify as benign, excluding features like age, gender, etc.
        for feature, strength in interconnection_strength_text.items():
            if strength in ["Very Strong", "Strong"]:
                # Check if the feature is present in the input data and not zero
                if input_data.get(feature) != 0:
                    if feature == "Smoking":
                        counterfactual.append("• The patient should quit smoking: Smoking significantly increases the risk of coronary artery disease by accelerating atherosclerosis and narrowing the arteries.")
                    elif feature == "Angiopathy":
                        counterfactual.append("• The presence of angiopathy should be addressed: Angiopathy, often associated with other conditions like diabetes and hypertension, can damage the blood vessels, exacerbating heart disease.")
                    elif feature == "known CAD":
                        counterfactual.append("• Managing known CAD could improve the outcome: Having a history of coronary artery disease necessitates careful management to reduce the likelihood of further complications and heart events.")
                    elif feature == "Diabetes":
                        counterfactual.append("• Better control of diabetes could reduce CAD risk: Diabetes increases the likelihood of plaque buildup in the arteries, exacerbating the risks of CAD.")
                    elif feature == "Hypertension":
                        counterfactual.append("• Managing hypertension more effectively could improve cardiovascular health: High blood pressure causes extra strain on the heart and arteries, increasing the chances of CAD and heart attacks.")
                    elif feature == "Family History of CAD":
                        counterfactual.append("• Addressing family history of CAD could reduce risk: A family history of CAD increases the chances of developing heart disease due to inherited risk factors.")
                    elif feature == "previous AMI":
                        counterfactual.append("• Managing previous AMI more effectively could improve the prognosis: A history of Acute Myocardial Infarction (heart attack) requires ongoing care to reduce the risk of recurrence.")
                    elif feature == "previous PCI":
                        counterfactual.append("• Managing previous PCI more effectively could improve the outcome: If the patient has had previous Percutaneous Coronary Intervention (PCI), monitoring and preventive care are essential to avoid further blockages.")
                    elif feature == "previous CABG":
                        counterfactual.append("• Managing previous CABG more effectively could improve the prognosis: Coronary Artery Bypass Grafting (CABG) is a serious procedure, and proper post-operative care can help improve long-term heart health.")
                    elif feature == "Dyspnoea on Exertion": 
                        counterfactual.append("• Managing dyspnoea on exertion more effectively could improve patient outcomes: Dyspnoea, or shortness of breath, during physical activity can be a sign of underlying cardiovascular or pulmonary issues, and addressing it can significantly enhance the patient's quality of life and overall health.")
        
        return "To classify this case as 'Normal', the following changes should be made:\n" + "\n".join(counterfactual)

    else:
        # print(input_data)
        # Suggest changes to classify as pathological, excluding features like age, gender, etc.
        for feature, strength in interconnection_strength_text.items():
            if strength in ["negative Very Strong", "negative Strong", "negative Medium"]:
                # Check if the feature is present in the input data and not zero
                if input_data.get(feature) == 0.0:
                    if feature == "Smoking":
                        counterfactual.append("• Smoking: Smoking is a major contributor to CAD risk and would significantly increase the likelihood of the disease.")
                    elif feature == "Angiopathy":
                        counterfactual.append("• Angiopathy: The presence of angiopathy in the blood vessels is a strong risk factor for CAD and must be carefully monitored.")
                    elif feature == "known CAD":
                        counterfactual.append("• Known CAD: A history of CAD substantially raises the risk of future cardiovascular events and heart disease.")
                    elif feature == "Diabetes":
                        counterfactual.append("• Diabetes: Uncontrolled diabetes accelerates plaque buildup and artery damage, significantly increasing the likelihood of CAD.")
                    elif feature == "Hypertension":
                        counterfactual.append("• Hypertension: High blood pressure is a key factor in the development and progression of coronary artery disease.")
                    elif feature == "Family History of CAD":
                        counterfactual.append("• Family History of CAD: If a family member has had heart disease, the likelihood of developing CAD increases due to inherited genetic factors.")
                    elif feature == "previous AMI":
                        counterfactual.append("• Previous AMI: A history of Acute Myocardial Infarction (heart attack) suggests an ongoing risk of heart issues and CAD.")
                    elif feature == "previous PCI":
                        counterfactual.append("• Previous PCI: Past interventions like Percutaneous Coronary Intervention (PCI) increase the risk of recurring blockages and CAD.")
                    elif feature == "previous CABG":
                        counterfactual.append("• Previous CABG: Having undergone Coronary Artery Bypass Grafting (CABG) implies severe coronary artery disease and raises the chance of future cardiovascular complications.")
    
        return "To classify this case as 'Pathological', the following attributes would increase the risk:\n" + "\n".join(counterfactual)
...
# best_position = best_position.to_numpy()  # Skip the first row which contains column names
...
for index, row in dataset.iterrows():
    ...
    # print(list(column_names), predicted_results)
    ...
    counterfactual_explanation = generate_counterfactual_explanation(input_data, interconnection_strength_text, classification)
